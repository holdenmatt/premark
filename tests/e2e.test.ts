...
describe('e2e', () => {
  loadMarkdownTests(testFile, (category, tests) => {
    describe(category, () => {
      tests.forEach(test => {
        it(test.name, async () => {
          const source = matter.stringify(test.context.document.content, test.context.document.frontmatter);
          const result = await compile(source, { resolver: test.context.resolver });
          
          // Parse both to compare
          const resultDoc = matter(result);
          const expectedDoc = matter(matter.stringify(test.expectedOutput!.content, test.expectedOutput!.frontmatter));
